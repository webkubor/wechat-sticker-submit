...
def main():
    ...
    ap.add_argument("src", nargs="?", help="源图目录（批量出 main_240/）")
    ap.add_argument("out")
    ...
    if args.src:
        # 横幅底图与 IP 原照混在同一个 raw/ 里，不能被当成表情图切进 main_240/
        ...
        print(f"表情图 {len(files)} 张 → {out}/main_240/")
        for i, f in enumerate(files, 1):
            save(fit_square(Image.open(f), 240), f"{out}/main_240/{i:02d}.png", 500)

    if args.cover:
        print("封面图 240×240（透明）")
        save(fit_square(Image.open(args.cover), 240), f"{out}/cover_240.png", 500)
    if args.icon:
        print("聊天面板图标 50×50（透明·头部正面）")
        save(fit_head(Image.open(args.icon), 50), f"{out}/icon_50.png", 100)
    if args.banner:
        print("详情页横幅 750×400（不透明·避免文字）")
        save(cover_crop(Image.open(args.banner), (750, 400)), f"{out}/banner_750x400.jpg", 500, jpeg=True)
    if args.reward_guide:
        print("赞赏引导图 750×560（不透明）")
        save(cover_crop(Image.open(args.reward_guide), (750, 560)).convert("RGB"),
             f"{out}/reward-guide_750x560.png", 500)
    if args.reward_thanks:
        print("赞赏致谢图 750×750（不透明）")
        save(cover_crop(Image.open(args.reward_thanks), (750, 750)).convert("RGB"),
             f"{out}/reward-thanks_750x750.png", 500)
