...
帧目录里放按文件名排序的透明 PNG（任意尺寸，会统一切成 240×240）。
...
def load_frames(src, side=SIDE, pad=0.94):
    files = sorted(f for f in glob.glob(os.path.join(src, "*"))
                   if f.lower().endswith((".png", ".webp", ".gif")))
    if not files:
...
def write_gif(frames, out, fps, colors, keep_every=1):
    seq = frames[::keep_every] if keep_every > 1 else frames
    conv = [rgba_to_p(f, colors=colors) for f in seq]
...
def main():
    ...
    files, frames = load_frames(args.src)
    print(f"读入 {len(frames)} 帧 → {SIDE}×{SIDE}")
