...
| 层 | 位置 | 放什么 |
|---|---|---|
| 全局依赖 | `~/.wechat-stickers/bgm/` 等**白名单**目录 | 跨形象复用且不可再生的（如配乐原曲） |
| 形象级 | `ips/<形象>/ip.yml` | 名称·简介·母版·画稿·已挂系列·待办 |
| 系列级 | `albums/<形象>/<系列>/album.yml` | 专辑文案·含义词·附加信息·推广文案·**平台状态** |

**不要新建旁挂目录。** 一套系列的所有配置都在它自己的 `album.yml` 里 ——
推广文案曾被放进旁挂的 `captions/`、平台状态曾被写进 `ip.yml`，
两次都是「改哪个文件就往哪写」的就近落盘，攒起来就是配置散落一地。

`scripts/lint.sh` 会检查数据根下有没有未登记的目录（白名单外一律 FAIL）——
要加新位置得先改白名单，这个动作本身迫使人重新想一遍分层。
